...
function mapTmdbSeriesStatus(s) {
    ...
    if (l.includes("canceled") || l.includes("cancelled"))
        return "canceled";
    if (l.includes("ended"))
        return "ended";
...
async function videoByName(input, apiKey) {
    ...
    if (!query)
        return null;
    const data = await tmdbFetch("/search/multi", apiKey, { query, include_adult: "true" });
    ...
    if (isMovie) {
        const detail = await tmdbFetch(`/movie/${best.id}`, apiKey, { append_to_response: "credits" });
...
async function videoByURL(input, apiKey) {
    ...
    if (parsed.mediaType === "movie") {
        const detail = await tmdbFetch(`/movie/${parsed.id}`, apiKey, { append_to_response: "credits" });
        return movieToVideoResult(detail);
    }
    else {
        const detail = await tmdbFetch(`/tv/${parsed.id}`, apiKey);
        return tvToVideoResult(detail);
    }
}
async function folderSeriesFromTvDetail(detail, input, apiKey, candidates) {
    ...
    if (local) {
        for (const loc of local) {
            let seasonDetail;
            try {
                seasonDetail = await tmdbFetch(`/tv/${detail.id}/season/${loc.seasonNumber}`, apiKey);
            }
            catch {
                seasonsOut.push({
...
async function folderCascade(input, apiKey) {
    ...
    const seasonDetail = await tmdbFetch(`/tv/${tvId}/season/${season}`, apiKey);
    ...
    const tvDetail = await tmdbFetch(`/tv/${tvId}`, apiKey);
    return {
...
// ─── Result builders ──────────────────────────────────────────────
function movieToVideoResult(detail) {
...
function tvToVideoResult(detail) {
    const network = detail.networks?.[0]?.name ?? detail.production_companies?.[0]?.name ?? null;
    const genres = (detail.genres ?? []).map((g) => g.name);
    return {
        title: detail.name,
