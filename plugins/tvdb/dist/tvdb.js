...
async function getToken(apiKey) {
    if (cachedToken && cachedToken.apiKey === apiKey)
...
async function tvdbFetch(path, apiKey, params = {}) {
    const url = new URL(`${TVDB_API}${path}`);
    for (const [k, v] of Object.entries(params)) {
        url.searchParams.set(k, v);
    }
    const doFetch = async (token) => fetch(url.toString(), {
...
function artworkCandidates(artworks, typeIds) {
    if (!artworks?.length)
        return [];
    ...
    return matches
        ...
        rank: typeof a.score === "number"
            ? Math.min(10, a.score / 1000)
            : 0,
        language: a.language ?? null,
        width: typeof a.width === "number" ? a.width : undefined,
        height: typeof a.height === "number" ? a.height : undefined,
        aspectRatio: typeof a.width === "number" && typeof a.height === "number" && a.height > 0
            ? a.width / a.height
            : undefined,
    }))
...
function firstArtwork(artworks, typeIds) {
    if (!artworks?.length)
        return null;
    for (const a of artworks) {
        if (typeIds.includes(a.type) && a.image)
            return a.image;
    }
    return null;
}
// ─── Status mapping ───────────────────────────────────────────────
function mapTvdbSeriesStatus(s) {
    if (!s)
        return null;
        ...
        if (Array.isArray(epsRaw)) {
            for (const e of epsRaw) {
                if (typeof e !== "object" || !e)
                    continue;
                const er = e;
                const en = typeof er.episodeNumber === "number"
                    ? er.episodeNumber
...
function parseTvdbUrl(url) {
    try {
        const u = new URL(url);
        if (!/thetvdb\.com$/.test(u.hostname.replace(/^www\./, "")))
            return null;
...
function searchHitToCandidate(hit) {
...
function extractTvdbIdOverride(input) {
...
async function folderSeriesFromSeriesExtended(detail, input, apiKey, candidates) {
    const genres = (detail.genres ?? []).map((g) => g.name).filter(Boolean);
    const network = detail.originalNetwork?.name ??
    ...
    const posterCandidates = mergeFallbackCandidate(artworkCandidates(detail.artworks, [ARTWORK_SERIES_POSTER]), detail.image ?? null, 5);
    ...
    if (local) {
        ...
        const officialSeasons = detail.seasons?.filter((s) => s.type.type === "official" || s.type.name.toLowerCase() === "aired order") ?? detail.seasons ?? [];
        for (const loc of local) {
            const seasonStub = officialSeasons.find((s) => s.number === loc.seasonNumber);
            let seasonDetail = null;
            if (seasonStub) {
                try {
                    seasonDetail = await tvdbFetch(`/seasons/${seasonStub.id}/extended`, apiKey);
                }
                catch {
                    seasonDetail = null;
                }
            ...
            const seasonPosterCandidates = mergeFallbackCandidate(artworkCandidates(seasonDetail?.artwork, [
                ARTWORK_SEASON_POSTER,
                ARTWORK_SEASON_BANNER,
            ]), seasonDetail?.image ?? seasonStub?.image ?? null, 4);
            const episodesOut = loc.episodes.map((le) => {
                const t = tvdbEps.find((e) => (e.seasonNumber ?? loc.seasonNumber) === loc.seasonNumber && e.number === le.episodeNumber);
    ...
    return {
        title: detail.name,
        originalTitle: detail.translations?.nameTranslations?.find((t) => t.language === detail.originalLanguage)?.name ?? null,
        ...
        imageUrl: firstArtwork(detail.artworks, [ARTWORK_SERIES_POSTER]) ?? detail.image ?? null,
...
async function folderByName(input, apiKey) {
    ...
    if (override !== null) {
        const detail = await tvdbFetch(`/series/${override}/extended`, apiKey, { meta: "translations" });
    ...
    if (!cleanQuery)
        return null;
    const hits = await tvdbFetch(`/search`, apiKey, {
        query: cleanQuery,
        type: "series",
        limit: "20",
    });
    ...
    const candidates = scored
        ...
        .filter((c) => c.externalIds.tvdb);
    const detail = await tvdbFetch(`/series/${bestId}/extended`, apiKey, { meta: "translations" });
...
async function videoByURL(input, apiKey) {
    ...
    if (parsed.episodeId) {
        const ep = await tvdbFetch(`/episodes/${parsed.episodeId}/extended`, apiKey);
        const series = ep.seriesId != null
            ? await tvdbFetch(`/series/${ep.seriesId}/extended`, apiKey).catch(() => null)
            : null;
    ...
    if (seriesId == null && parsed.seriesSlug) {
        ...
        if (slugHit)
            seriesId = searchHitId(slugHit);
    }
    if (seriesId == null)
        return null;
    const detail = await tvdbFetch(`/series/${seriesId}/extended`, apiKey, { meta: "translations" });
    return folderSeriesFromSeriesExtended(detail, input, apiKey);
}
async function folderCascade(input, apiKey) {
    ...
    // fetch episodes.
    const detail = await tvdbFetch(`/series/${seriesId}/extended`, apiKey);
    const seasonStub = (detail.seasons ?? []).find((s) => s.number === seasonNumber &&
        (s.type.type === "official" || s.type.name.toLowerCase() === "aired order"));
    if (!seasonStub)
        return null;
    const seasonDetail = await tvdbFetch(`/seasons/${seasonStub.id}/extended`, apiKey);
