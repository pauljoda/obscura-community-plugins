 ...
 * empty.
 */
function thumbCandidates(video: YTVideoItem): ImageCandidate[] {
  ...
  const staticFallback = (variant: string, rank: number) => {
...
function bestThumbUrl(video: YTVideoItem): string | null {
  const t = video.snippet.thumbnails;
  return (
    t.maxres?.url ??
    t.standard?.url ??
...
function videoToResult(video: YTVideoItem): Record<string, unknown> {
...
async function videoByName(
  ...
  const q = ((input.name as string) ?? (input.title as string) ?? "").trim();
  if (!q) return null;

  const search = await ytFetch<{ items: YTSearchItem[] }>(`/search`, apiKey, {
    part: "snippet",
    q,
    type: "video",
    maxResults: "5",
  ...
  if (!firstWithId?.id.videoId) return null;
...
async function videoById(
  ...
  const data = await ytFetch<{ items: YTVideoItem[] }>(`/videos`, apiKey, {
    id,
...
async function videoBatchByURL(
  ...
  const idByItem = new Map<string, string | null>();
  for (const item of items) {
    const url = (item.input.url as string) ?? "";
    idByItem.set(item.id, extractVideoId(url));
  }

  const ids = [...new Set([...idByItem.values()].filter((v): v is string => Boolean(v)))];
  const byYouTubeId = new Map<string, YTVideoItem>();
  ...
  for (let i = 0; i < ids.length; i += 50) {
    ...
    for (const v of data.items ?? []) byYouTubeId.set(v.id, v);
  }

  return items.map((item) => {
    const ytId = idByItem.get(item.id) ?? null;
    if (!ytId) return { id: item.id, result: null };
