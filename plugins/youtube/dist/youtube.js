 ...
 */
function idFromInput(input) {
    const fields = ["url", "filePath", "name", "title", "details"];
...
function thumbCandidatesFromInnertube(videoId, thumbs) {
    const out = [];
    ...
    for (const t of sorted) {
        ...
        out.push({
            ...
            width: t.width,
            height: t.height,
            aspectRatio: t.width && t.height && t.height > 0 ? t.width / t.height : undefined,
        });
...
async function innertubePost(endpoint, body) {
    const res = await fetch(`${YT_INNERTUBE_BASE}/${endpoint}?key=${INNERTUBE_WEB_KEY}&prettyPrint=false`, {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
        throw new Error(`YouTube InnerTube /${endpoint} error: ${res.status} ${res.statusText}`);
    }
...
async function innertubePlayer(videoId) {
    ...
    if (data.error) {
        throw new Error(`YouTube InnerTube error: ${data.error.code ?? "?"} ${data.error.message ?? "unknown"}`);
    }
...
async function innertubeSearchTopVideoId(query) {
    const data = await innertubePost("search", {
        query,
        context: INNERTUBE_CONTEXT,
    });
    if (data.error) {
        throw new Error(`YouTube InnerTube search error: ${data.error.code ?? "?"} ${data.error.message ?? "unknown"}`);
    }
    const sects = data.contents?.twoColumnSearchResultsRenderer?.primaryContents
...
// ─── Result builders ──────────────────────────────────────────────
function parseLengthSeconds(s) {
    if (!s)
        return null;
    const n = Number.parseInt(s, 10);
    return Number.isFinite(n) && n > 0 ? n : null;
}
function resultFromInnertube(data) {
    ...
    const thumbs = thumbCandidatesFromInnertube(vd.videoId, vd.thumbnail?.thumbnails);
...
async function resolveById(videoId) {
    ...
    try {
        ...
        console.error(`[youtube] InnerTube /player failed, falling back to oEmbed: ${err instanceof Error ? err.message : String(err)}`);
    }
...
async function videoByName(input) {
    ...
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`[youtube] InnerTube /search failed: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
    if (!id)
        return null;
 ...
 */
async function batchResolve(items, resolver) {
    const out = new Array(items.length);
    let next = 0;
    const workers = Array.from({ length: Math.min(BATCH_CONCURRENCY, items.length) }, async () => {
        while (true) {
            const i = next++;
            if (i >= items.length)
                return;
...
exports.default = {
    ...
    async executeBatch(action, items) {
