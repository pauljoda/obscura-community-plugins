...
function parseDescriptionTags(text) {
    ...
    const tagsIndex = lines.findIndex((line) => line.replace(/[|\s]/g, "").toLowerCase() === "tags");
    if (tagsIndex === -1) {
...
function candidatesFromMangaList(manga) {
    const candidates = [];
...
function coverCandidates(manga, covers, preferredUrl) {
    const candidates = [];
